...
4. **CorrelationEngine** (`src/core/correlation_engine.py`) — sliding-window
   ...
   meta-detections (`entity_persistence`, `cross_sensor`, `burst`).
...
5. **Outputs** — every detection fans out to the `ForensicLogger` (JSONL,
   ...
   egress allowlist), and the in-memory `DetectionStore` that backs the
   dashboard's `/detections` API.
6. **Dashboard** (`src/dashboard/app.py`) — FastAPI app that owns the
   ...
   forensic/alert/correlation flow. Bearer-token auth (`server.api_token` or
